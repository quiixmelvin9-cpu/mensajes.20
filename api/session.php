...
$userStmt = $conn->prepare('SELECT id, nombre_usuario, nombre_completo FROM usuarios WHERE id = ? LIMIT 1');
$userStmt->bind_param('i', $userId);
$userStmt->execute();
$currentUser = $userStmt->get_result()->fetch_assoc();
$userStmt->close();

$listStmt = $conn->prepare('SELECT id, nombre_usuario, nombre_completo FROM usuarios WHERE id <> ? ORDER BY nombre_usuario ASC');
